document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.testimonial-read-more').forEach((button) => {
    ...
    const testimonialObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
            if (entry.isIntersecting) { entry.target.classList.add('in-view'); testimonialObserver.unobserve(entry.target); }
        });
    }, { threshold: 0.15 });
